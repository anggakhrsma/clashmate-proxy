...
exports.initializePersistence = initializePersistence;
...
const migrations = [initialSchemaMigration, apiKeysMetadataMigration];
function timestamp() {
...
function mapDeveloperAccountRow(row) {
    return {
        id: row.id,
        slot: row.slot,
        email: row.email,
...
function resolveDatabasePath(sqlitePath) {
    const resolvedPath = (0, node_path_1.resolve)(sqlitePath);
...
function applyMigrations(database) {
  ...
  `);
    ...
    const getMigration = database.prepare('SELECT name FROM schema_migrations WHERE name = :name');
    const insertMigration = database.prepare(`
    INSERT INTO schema_migrations (name, executed_at)
    VALUES (:name, :executedAt)
  `);
    for (const migration of migrations) {
        const existing = getMigration.get({ name: migration.name });
        if (existing) {
            continue;
        }
        database.exec('BEGIN');
...
class SqlitePersistence {
    ...
    constructor(databasePath) {
        ...
        this.database = new node_sqlite_1.DatabaseSync(this.databasePath);
        this.appliedMigrations = applyMigrations(this.database);
    }
    close() {
        this.database.close();
    }
    syncConfiguredDeveloperAccounts(accounts) {
        const now = timestamp();
    ...
    listDeveloperAccounts() {
      ...
      ORDER BY slot ASC
    `);
        return statement.all().map(mapDeveloperAccountRow);
    }
    updateDeveloperAccountStatus(input) {
    ...
    `);
        statement.run({
            slot: input.slot,
            isEnabled: toBooleanInteger(hasOwnKey(input, 'isEnabled')
                ? (input.isEnabled ?? account.isEnabled)
                : account.isEnabled),
            isHealthy: toBooleanInteger(hasOwnKey(input, 'isHealthy')
        ...
        if (!updatedAccount) {
            throw new Error(`Developer account slot ${input.slot} disappeared after update.`);
        }
        return updatedAccount;
    }
    getDeveloperAccountBySlot(slot) {
        const statement = this.database.prepare(`
      SELECT
        id,
        slot,
    ...
    getApiKeyByValue(keyValue) {
      ...
      LIMIT 1
    `);
        const row = statement.get({ keyValue });
        return row ? mapApiKeyRow(row) : null;
    }
    getApiKeyByAccountAndPortalKeyId(developerAccountId, portalKeyId) {
        const statement = this.database.prepare(`
      SELECT
        id,
        developer_account_id,
    ...
    listApiKeys() {
      ...
      ORDER BY id ASC
    `);
        return statement.all().map(mapApiKeyRow);
    }
    updateApiKeyStatus(input) {
    ...
    `);
        statement.run({
            ...
            keyName: hasOwnKey(input, 'keyName')
                ? (input.keyName ?? null)
                : apiKey.keyName,
            cidrRangesJson: JSON.stringify(hasOwnKey(input, 'cidrRanges')
                ? (input.cidrRanges ?? [])
                : apiKey.cidrRanges),
            isManaged: toBooleanInteger(hasOwnKey(input, 'isManaged')
                ? (input.isManaged ?? apiKey.isManaged)
                : apiKey.isManaged),
            isActive: toBooleanInteger(hasOwnKey(input, 'isActive')
    ...
    listLifecycleEvents(limit = 50) {
      ...
      LIMIT :limit
    `);
        return statement.all({ limit: safeLimit }).map((row) => ({
            id: row.id,
            developerAccountId: row.developer_account_id,
            apiKeyId: row.api_key_id,
            eventType: row.event_type,
...
function initializePersistence(input) {
    const persistence = new SqlitePersistence(input.sqlitePath);
    const syncedAccounts = persistence.syncConfiguredDeveloperAccounts(input.developerAccounts).length;
    return {
        persistence,
        bootstrap: {
