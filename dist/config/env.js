...
function parsePositiveInteger(name, value, fallback, errors) {
    if (!value) {
        return fallback;
    }
    const parsed = Number.parseInt(value, 10);
...
function parseNonNegativeInteger(name, value, fallback, errors) {
    if (!value) {
        return fallback;
    }
    const parsed = Number.parseInt(value, 10);
...
function parseRequiredString(name, errors) {
...
function parseStringList(value, separator = ',') {
    if (!value) {
        return [];
    }
    return value
...
function parseRequiredStringList(name, value, errors) {
...
function parseEnumValue(name, value, allowedValues, fallback, errors) {
    if (!value) {
        return fallback;
    }
    ...
    errors.push(`${name} must be one of: ${allowedValues.join(', ')}. Received: ${value}`);
    return fallback;
}
function parseUrl(name, value, fallback, errors) {
...
function parseCocDeveloperAccounts(errors) {
    const accounts = [];
    ...
    for (let slot = 1; slot <= MAX_COC_ACCOUNTS; slot += 1) {
        ...
        if (!email || !password) {
            errors.push(`COC_ACCOUNT_${slot}_EMAIL and COC_ACCOUNT_${slot}_PASSWORD must both be provided together.`);
            continue;
        }
        if (!email.includes('@')) {
            errors.push(`COC_ACCOUNT_${slot}_EMAIL must be a valid email address. Received: ${email}`);
            continue;
        }
...
function loadEnv() {
    ...
    const nodeEnv = parseEnumValue('NODE_ENV', readEnvValue('NODE_ENV'), allowedNodeEnvironments, DEFAULT_NODE_ENV, errors);
    const logLevel = parseEnumValue('LOG_LEVEL', readEnvValue('LOG_LEVEL'), allowedLogLevels, DEFAULT_LOG_LEVEL, errors);
    ...
    const cacheTtlSeconds = parsePositiveInteger('CACHE_TTL_SECONDS', readEnvValue('CACHE_TTL_SECONDS'), DEFAULT_CACHE_TTL_SECONDS, errors);
    const validationSweepIntervalMinutes = parsePositiveInteger('VALIDATION_SWEEP_INTERVAL_MINUTES', readEnvValue('VALIDATION_SWEEP_INTERVAL_MINUTES'), DEFAULT_VALIDATION_SWEEP_INTERVAL_MINUTES, errors);
    const upstreamBaseUrl = parseUrl('UPSTREAM_BASE_URL', readEnvValue('UPSTREAM_BASE_URL'), DEFAULT_UPSTREAM_BASE_URL, errors);
    const upstreamTimeoutMs = parsePositiveInteger('UPSTREAM_TIMEOUT_MS', readEnvValue('UPSTREAM_TIMEOUT_MS'), DEFAULT_UPSTREAM_TIMEOUT_MS, errors);
    ...
    if (clientApiSecret && adminApiSecret && clientApiSecret === adminApiSecret) {
        errors.push('CLIENT_API_SECRET and ADMIN_API_SECRET must be different values.');
    }
    if (errors.length > 0) {
